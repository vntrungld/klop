...
**Architecture:** A new Qt-free CLI command `clop-kde optimize-url <url>` downloads the remote image (stdlib `urllib`), validates it, saves it (dedup) to `web_drop_dir`, optimizes it in place with the existing `Engine` (so it is undoable), copies the result to the clipboard via `wl-copy`/`xclip`, records history, and notifies. The plasmoid's `onDropped` classifies each drop: `file://` → `optimize` (unchanged), `http(s)://` → `optimize-url`.
...
- **Fetch safety:** `http`/`https` only; 15s timeout; 50 MB cap; `User-Agent: clop-kde`; the bytes must validate as a real image by magic bytes (PNG/JPEG/GIF/WEBP/HEIC) or the fetch is rejected. Never overwrite an existing file (dedup suffix).
- **Single source of truth:** the plasmoid shells out to the CLI; it does not download or write files itself.
- **New config key:** `web_drop_dir: str` default `"~/Pictures/Klop"`, editable via `clop-kde config set web_drop_dir=…` (not in the QML settings form in v1).
...
**Files:**
- Create: `src/clop_kde/webfetch.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'clop_kde.webfetch'`.

- [ ] **Step 3: Implement `src/clop_kde/webfetch.py`**
...
```bash
git add src/clop_kde/webfetch.py tests/test_webfetch.py
...
- Produces: CLI verb `clop-kde optimize-url <url>`.
...
**Files:**
- Modify: `src/clop_kde/plasmoid_pkg/contents/ui/main.qml`
- Test: `tests/test_plasmoid_pkg.py`

**Interfaces:**
- Consumes: `Backend.CLOP_BIN`, `shquote`, `exec.run`, `refreshHistory` (existing in main.qml).
...
- [ ] **Step 3: Edit `src/clop_kde/plasmoid_pkg/contents/ui/main.qml`**
...
Then (best-effort): `kpackagetool6 --type Plasma/Applet --show src/clop_kde/plasmoid_pkg` — report result; skip if the tool is absent.
...
```bash
git add src/clop_kde/plasmoid_pkg/contents/ui/main.qml tests/test_plasmoid_pkg.py
...
Run: `clop-kde optimize-url https://upload.wikimedia.org/wikipedia/commons/3/3f/JPEG_example_flower.jpg`
Expected: prints an `optimized …`/`unchanged …` line and `saved /home/…/Pictures/Klop/JPEG_example_flower.jpg`; the file exists there; if `wl-copy` is installed, the image is on the clipboard (paste to confirm); `clop-kde history` shows the new row.
...
Run: `clop-kde optimize-url https://example.com/` (an HTML page, not an image).
