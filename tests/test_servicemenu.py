...
from clop_kde import servicemenu


def test_desktop_entry_wires_exec_action_and_mimetypes():
    entry = servicemenu.desktop_entry("/opt/bin/clop-kde", icon="/x/icon.svg")
    ...
    # required key
    assert entry.split("[Desktop Action", 1)[0].count("Name=Optimize with Clop") == 1
    # the CLI backend is reused as the service-menu backend; %F passes the
    # selected local files
    assert 'Exec="/opt/bin/clop-kde" optimize %F' in entry
    assert "Actions=optimizeWithClop;" in entry
    assert "[Desktop Action optimizeWithClop]" in entry
    assert "Name=Optimize with Clop" in entry
...
def test_install_writes_executable_desktop_file(tmp_path):
    dest = servicemenu.install(exec_path="/opt/bin/clop-kde", dest_dir=tmp_path)

    assert dest.parent == tmp_path
    assert dest.suffix == ".desktop"
    text = dest.read_text()
    assert 'Exec="/opt/bin/clop-kde" optimize %F' in text
...
def test_install_creates_missing_dest_dir(tmp_path):
    ...
    dest = servicemenu.install(exec_path="/opt/bin/clop-kde", dest_dir=nested)
    assert dest.exists()
